...
def codegen_lua_from_program(
    ...
    *,
    ...
    point_prefix: str = 'p',
) -> tuple[str, dict[str, dict[str, list[float]]], int]:
    ...
    Points are emitted as
        p1 = {joint = {j1, j2, j3, j4, j5, j6}}
    on the varspoint side. The Lua source references them by name
    only (`movJ(p1)`), which is exactly what varspoint.json is for.
    """
    ...
    for step in steps:
        ...
        if not (isinstance(taught, list) and len(taught) == 6
            ...
            lines.append(f'-- skipped {action!r}: '
                         f'no 6-element taught_joints (got {type(taught).__name__})')
            continue
        idx += 1
        name = f'{point_prefix}{idx}'
        varspoint[name] = {'joint': [float(v) for v in taught]}
        # Comment shows the action + taught angles so a reviewer can
        # sanity-check the generated program against the source.
        joints_s = ', '.join(f'{float(v):+.3f}' for v in taught)
        lines.append(f'-- step[{idx}] action={action!r}  joints=[{joints_s}]')
        lines.append(f'movJ({name})')

    trailer = time.strftime(_LUA_TRAILER_FMT, time.localtime(time.time()))
    lines.append('')
    lines.append(trailer)
    # The demo file uses CRLF line endings. Match that so the diff on
    # the controller side stays clean.
    source = '\r\n'.join(lines) + '\r\n'
