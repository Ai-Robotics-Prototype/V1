...
**D11. Column orientation lock.**
Within each station column (approach-above → contact → retreat-above),
the TCP orientation MUST be identical to the taught contact anchor's
orientation, within 0.1° on the max-axis of R_anchor · R_derived^T.
Enforced BY CONSTRUCTION at codegen: derived approach/retreat poses
are generated by a full 6-DOF Newton IK
(`seeded_ik_z_lift_hold_orientation`) that targets the anchor's
base-frame rotation matrix; the older 3-DOF Z-arm-only lift (which
held q4/q5/q6 but let q1/q2/q3 rotate all downstream frames, drifting
the flange by up to ~9°) is retained only for movJCoorRel fallbacks
outside a column. Orientation may change ONLY on the transit movL
between retreat-above(A) and approach-above(B) — smoothly, in the
air, between stations. A program with any column-orient-delta above
0.1° cannot save: `analyze_program` emits severity `'block'` and both
POST/PUT `/api/programs` refuse the write. Anchor tilt-from-vertical
> 3° surfaces as an INFO finding at teach time — a crooked teach is
visible before it's discovered on camera. _(Source: 2026-08-03
operator directive after paired-evidence check; column-orientation-
lock IK convergence proven on the bowl program with residuals
0.000134° / 0.000029° for PICK / PLACE.)_
...
| D11 | D11_column_orient_lock.test.js | Compose a pick&place fixture with a taught contact; codegen the derived approach + retreat via `seeded_ik_z_lift_hold_orientation`; FK all three poses; assert max-axis of R_anchor·R_derived^T ≤ 0.1° on both sides. Analyzer must report severity='block' with rule 'column_orient_delta' when the tolerance is exceeded. Save gate: `_d11_block_findings` returns non-empty on a synthesized rotating-column program. |
