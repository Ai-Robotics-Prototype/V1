...
302. Under WS-jog, the tab-kill deadman fires via the dashboard's WS-disconnect handler translating `WebSocketDisconnect(1001, '')` into an explicit release payload on `/robot/jog_command`; driver logs `cause=release_cmd` (NOT `cause=freshness_deadman`). End-to-end operator-refresh → wire `Robot/stopJog` measured at ~50 ms; the 200 ms freshness deadman is the backstop-of-backstop, not the primary path. Phantom-defense chain (blacklist + JOG-3 readopt-reject + JOG-11 arbiter) prevents a tab reconnect from adopting the pre-death hold_id — verified on-wire: 96 START/RELEASE pairs, zero new events after the tab-kill. — add-47 §610 — add-47 §610
303. Motion arbiter live-fire must test the direction the pytest cannot lie about: state-propagation. Direction 1 (run-during-active-jog → 409) reads `_active_holds` which is real by construction — pytest can inject state, but live-fire proves the reader sees the writer's state. Direction 2 (jog-during-running-program → 409) reads `STATE.robot.program.state` populated by `_on_estun_program_status` from a `/estun/program_status` topic message — this path can lie under a bad implementation, so it needs to fire against a genuinely running program, NOT injected state. Folding direction 2 into the F2.7 first-taught-program milestone closes both directions in one operator session without requiring a synthetic ALLOW_MOVE lift for a spurious "wait-only" program (codegen rejects programs with zero mov* verbs). — add-47 §611
304. Twin phantom-feedback class ended by structural fix: bind feedback consumption to JOG_BACKEND (each mode has ONE authoritative source; the other feed is IGNORED-and-counted) + all-zeros quarantine safety net (any exact-zero frame under `state=2 Enabled` is impossible telemetry, rejected + counted). Root cause of this incident: dashboard's `_on_joint_states` looked up canonical `Joint1..Joint6` (CRI-JSB naming) but roboai-estun publishes `joint_1..joint_6` (lowercase); the `pos_by_name.get(n, 0.0)` fallback wrote all zeros to `STATE.joints.positions` every frame, racing `_on_estun_status`'s real writer. Class history: #1 add-40 §537 L251 (silent mock, synthetic zeros), #2 add-45 §596 hunt (JSB unconfigured), #3 THIS (dashboard case-mismatch racing WS mirror). Doctrine test 13/13 pins both the bindings and the quarantine — any future phantom must clear BOTH gates. — add-47 §612
307. WS-jog guard demotion — streamed-era vs verb-era trust boundary. When the driver sent 250 Hz joint-position setpoints on UDP `:9030` (CRI-day, add-32 §506), WE enforced every limit — axis clamp, joint-velocity cap, σ_min guard, angular soft governor. On the `Robot/jog` WS verb (add-16 §281 + add-19), the CC10-A firmware sits BETWEEN us and the motion: it does the IK, clamps `db.speed` per-joint at the axis, refuses travel past a physical limit, and slows without erroring at singularities — proven by the factory pendant showing zero mid-hold stops on the identical wound-wrist pose that killed our client. The driver still had the streamed-era guards running on the verb-era path; they tripped early and killed the hold before the firmware's own better-tuned response engaged. Fix: feature gate `wsjog_trust_firmware_clamps` (default True) demotes six cart-mode guards to observe-only — `cart_limit_at_wall`, `cart_limit_deepening`, `joint_limit_soft`, `joint_overspeed`, `singularity_guard`, `sigma_soft`. Under observe, `cart_softening.mode='observe'` populates for the toast; no `_stop_jog_locked`, no `_apply_cart_speed_scale_locked`. Kept enforced: `collision_guard` (self/ground/env — our unique layer, firmware knows nothing about our capsule model or ground plane), freshness deadman (firmware can't detect browser death), arbiter (JOG-11 architectural policy), JOINT-mode `escape_only` + JointRecoveryModal (UX guard the operator asked for in add-42), and true faults/alarms/gestures. Env override `WSJOG_TRUST_FIRMWARE_CLAMPS=0` restores ENFORCE for regression. Doctrine tests pin the trust gate + enumerate the observe causes + enumerate the KEEP set. Parity bar: side-by-side factory-UI vs our-UI on the same wound-wrist cart hold shows identical behavior — same slowdowns, same limit stops, zero extra stops from our layer. — add-50 §630-637
306. Mode switching (Manual/Auto/Remote) belongs on the operator's dashboard, gated separately from program-write. Structural shape: driver `allow_mode` / `ESTUN_ALLOW_MODE` gate (separate from `allow_move`) → subscription `/estun/mode_command` → WS verb (`Robot/toAuto`|`toManual`|`toRemote`, all pre-captured in add-19 / add-25) → **numeric** read-back verify against `_robot_mode_code` (0/1/2, from `publish/RobotStatus.mode`, L298 ground truth) over 3 s at ~10 Hz sample → result envelope on `/estun/mode_status`. Dashboard `POST /api/estun/mode` extends JOG-11 arbiter discipline (refuse under `_active_holds` > 0 OR `program.state == 2`, named `arbiter_refused`), publishes with a UUID `req_id`, waits ≤ 4 s for a matching envelope, emits event_log entries on both success (`code=mode_switch`) and refusal (`code=mode_switch_refused`) so the operator timeline is complete. Frontend `ModeControl.jsx` renders the pill ALWAYS in the RealArm chrome (safety-relevant, not a toggle) with named consequence per target in the confirm dialog; `RunProgramModal` transparently reads "Switch to Auto and run at N%" and does the switch-then-run in one confirmed click. Wire truth: `Robot/toAuto sent ok=True` (WS ack) is NOT a mode transition — only `robot_mode_code` reaching the target counts. Live 2026-08-28: verb sent, mode did not transition within 3 s; two candidate controller preconditions (pendant key-switch position, arm-disabled requirement) unresolved. That's an honest refusal the endpoint surfaces; not a plumbing bug. — add-49 §621-629
308. Mode-switch refusal diagnostic ladder — §566 four-tuple is ground truth, not just for CRI. Symptom that ran six hours: `Robot/toAuto sent ok=True` on the WS (verb ack success), `Robot/switchOff/On` orchestration ok, but `publish/RobotStatus.mode` never transitioned 1→0. Every mode-switch endpoint call returned `mode_readback_timeout`. Blind spot: driver's `_on_status` parsed `state/stateName/mode/estop/moving` but NOT `errors[]` OR `recoveryState` — the exact fields addendum-40 §566 pinned as ground truth for recovery-state observation. Six-month-old ledger, forgotten mirror. Actual state: `recoveryState=1, errors=[]` — §566 called out that `System/ClearError + Robot/switchOn` clears the errors list AND returns state to 2 BUT does NOT clear `recoveryState`. Physical cabinet power-cycle is the ONLY remedy. Fix: driver parses both from `db.get('errors')` / `db.get('recoveryState')`; dashboard `_on_estun_mode` mirrors both keys; `/api/estun/mode` gains Rung 1 (recoveryState!=0 → refuse immediately with `reason_code=recovery_state_nonzero` naming the physical cabinet cycle, HTTP 503, no orchestration attempted — wire retry would burn 4 s on a state the wire cannot fix) and Rung 2 (errors[] non-empty → publish clear_alarm, poll for drain 2 s, retry; on persistent → `errors_latched_uncleared`). Every terminal response carries the §566 four-tuple in `outcome.four_tuple`. Doctrine rule: "Switch was refused" with no cause is a doctrine-test failure from now on. — add-51 §638-645
309. ROS2 executor cutover — the fix for a bug class can be the retirement of the code path that hosts it. Palletize `recoveryState=1` latching on EVERY push (add-51 §640) proved the class isn't a one-off; a Lua-side patch would trade fresh cabinet cycles for diagnosis time indefinitely. The class CANNOT exist on the CRI path (`s10_140_executor` package, Pilz PTP/LIN on UDP :9030) — no Lua, no HTTP push, no `resident_program_id`, no `MOVE_TO_PALL` runtime verb. Directive: F2.7 first-run acceptance IS the escape from the class; scaffolding flag `RUN_BACKEND=legacy_lua|ros2_executor` (default legacy until acceptance) with target mode auto-derived (AUTO for legacy, REMOTE for ros2 per HARDWARE.md > Robot-mode code table); `/api/provenance` publishes both fields; `RunProgramModal` reads them and auto-offers "Switch to <target> and run" via the existing add-51 orchestration. ros2 branch returns 501 `ros2_executor_not_wired_yet` (labeled stub) until F2.7 lands the action client — silent fallthrough would defeat the acceptance signal. §644 forensic codegen diff runs OFFLINE only (cheap, no arm cycles); its findings become the tombstone on the legacy path. NO more live reproduction cycles: the fix is the cutover, not a Lua patch. Palletize quarantine at the endpoint stays ON regardless of RUN_BACKEND — belt-and-braces protection while the flag is manipulable. §644 offline finding: palletize codegen catches an IK failure ("transit_over_slot [0,0,0] unreachable at Z=273.2mm — refusing") in `program_ops.py:3882+` pallet expansion but STILL emits a `movJ(p4)` — the shipped Lua is a partial/inconsistent expansion the controller executes into `recoveryState=1`. That specific defect is architecturally impossible on CRI: L222 pre-submit validation refuses the composite BEFORE any trajectory is dispatched. Tombstone documented; no fix on legacy. — add-52 §646-654
317. F2.7 first-fire — silent-refusal guard proven on real arm; blocked by J1/J6 convention drift (2026-08-31 add-55 §672-678). Full-day session across two repos. Real Test100 first-fire attempted at 25% speed after CRI bring-up + REMOTE acquisition + Test100 dry pass (10 plan_summaries) + arm-preflight CLEAR + explicit operator "go". Silent-refusal guard caught a mid-motion halt on step 0 (29° J4 gap between JTC reference and observed feedback) → executor emitted ERROR + halted, no cascade. Root cause of the halt: **operator e-stop** (WS `publish/Error` alarm 2006 "Emergency stop button pressed." at ts=1788202611.539). Every guard behaved correctly. Post-halt geometric investigation (FK'd planned trajectory: 500mm TCP travel + 41° J4 rotation + 459mm Z-rise) revealed the arm was moving in the correct direction toward taught home — the motion was startling because the arm started far from home, not wrong-direction. Real learnings from the fire: (1) **`run_speed_pct` was IGNORED for move_linear steps** — per-step Test100 speed_pct (60%, 30%) OVERRODE the top-level cap; FIX: MIN semantics, `vel_scale = min(per_step, run_cap)/100`. (2) **IK-seeded-from-current-arm produced 172° J4 wrist flips** on derived approach/retreat steps; PARTIAL FIX (seed from reference taught_joints) moved the flip to J5. FULL FIX: FK the reference step's taught_joints via `/compute_fk`, pass the resulting orientation quaternion as an override to IK (bypasses the axis-angle-as-XYZ-RPY convention bug in `_rpy_to_quat`). After fix: max joint delta 172° → 29° on all derived steps; wrist-flip class RESOLVED. (3) **Large-motion advisory** emits before plan_summary when joint delta > 20° OR TCP travel > 200mm; caught 5/8 motion steps in the current pose. (4) **Numeric plan_summary** reports peak_velocities vs joint_max × v_scaling with PROPAGATED/UNDER-SCALED verdict — operator can review speed BEFORE consenting. **First-fire retry BLOCKED by discovery: J1/J6 signs drift across concurrent WS clients AND across CRI teardown cycles.** Two clients subscribing to the same `publish/RobotPosture` topic return OPPOSITE sign for J1/J6 at the same instant on the same physical arm. UDP push sign flipped across a teardown+relaunch. Attempted fix in `cri_udp_system.cpp` (kUdpJointSigns adapter on read+write) — took effect per debug log but couldn't resolve the estun_driver-side inconsistency independently — REVERTED. `kUdpJointSigns` array + SIGN_ADAPTER debug log kept in code as reference for next-session investigation. Documented in `cobot-jsb-j1-j6-convention-drift` memory + STATE.md next-session opener with 7-step investigation plan. Also landed in this session: 4 agent charters + hooks under `.claude/agents/` (goal-auditor / enforcement-reviewer / session-close-auditor / arm-preflight) per background directive — pure judgment surfaces, all grep-checkable invariants stay in `fork_lint.py`; hooks wire SessionStart+Stop+PreToolUse. — add-55 §672-678

316. F2.7 acceptance — executor stops being a skeleton, RUN_BACKEND=ros2_executor wired end-to-end (2026-08-31, add-54 §665-671). Two commits, two repos, one atomic milestone. **CRI-side** (`CodroidROS2` main → `62ac884`): three new pure-logic modules — `ws_probe.py` (sync WS four-tuple probe against ws://192.168.2.136:9000; recoveryState observed not gated per add-53 §658; optional require_mode gate); `step_dispatch.py` (canonical kinds + aliases move_home→move_j / move_linear→move_l / set_io→set_do; `derived_from` + `offset_z_mm` resolves against position_role-tagged steps; J3/J5 sign-flip pendant→URDF per `cobot-cri-axis-convention`; round-trip verified 0.0 diff); `motion_plan.py` (MoveGroupPlanner with Pilz PTP for MoveJ + LIN for MoveL against `manipulator` group, workspace bounds from URDF, JointConstraint ±0.001 rad + PositionConstraint 1cm box + OrientationConstraint 0.05 rad; JTCExecutor with Humble cancel-terminal quirk workaround per `cobot-jtc-humble-cancel-terminal-quirk` — 500ms outer deadman, goal_handle.status as ground truth). Executor node rewrite: real `_ws_four_tuple_ok`, vocabulary resolution early (unresolvable = program refusal not mid-run failure), loop handling with `max_loop_iterations=1` first-run cap (Test100's count=0 reads as infinite), full motion path validate→plan→plan_summary→(dry_run?)→execute→settle→silent_refusal→step_verdict, publishes to NEW `/executor/status` topic (moved off `/estun/program_status` to avoid legacy-shape collision). **Dashboard-side** (`cobot_ws` feature/estun-write-path → NEXT SHA below): `/api/estun/program/run` ros2_executor branch — 501 stub DELETED, replaced with per-req_id awaiter bridge (`_register_executor_awaiter` + `_publish_task_run_program` + `_on_executor_status`). Publisher on `/task/run_program` (eager init, DDS discovery-race fix); subscriber on `/executor/status` translates `program_state ∈ {2,3}` → legacy `STATE.robot.program.state` so JOG-11 arbiter refuses jog during executor runs without a separate code path. Terminal outcomes: `executor_complete` / `executor_dry_run_complete` (200) / `executor_error` (409, with reason_code + step_idx + summaries) / `executor_timeout` (504) / `executor_not_running` (503, fast-fail when publisher has 0 subscribers). Palletize quarantine check preserved before dispatch (belt-and-braces per add-52 even though the class is architecturally impossible on CRI). **Drop-in**: `src/cobot_bringup/systemd/roboai-dashboard.service.d/f27-ros2-executor.conf` with `Environment=RUN_BACKEND=ros2_executor` — install by operator (deliberate, not auto-deploy). **Doctrine**: `test_run_endpoint_dispatches_on_run_backend` rewritten (retired stub kind MUST NOT appear; bridge helpers all present; quarantine precedes bridge; terminal-outcome kinds pinned); two new tests `test_executor_bridge_subscribes_executor_status_topic` + `test_executor_bridge_publisher_topic_and_dropin_present`. Fork-registry `jog_hold_heartbeat` line numbers refreshed (110-line shift). 91/91 dashboard doctrine + 24/24 executor gate tests + fork_lint rc=0 across both repos. **Deferred, non-blocking**: pause/resume, `/estun/io` set_io ack, true infinite loop, executor systemd unit auto-launch, frontend surface for plan_summaries[]/step_verdicts[] events (currently returned in HTTP body only). — add-54 §665-671, 2026-08-31

315. Mode switching via bound-DI software binding — DI16 hunt RETIRED (2026-08-31 software manual absorb, add-53 §663 follow-up, add-54 shipped). Second manual absorb of the day revealed the actual mechanism: the CC10-A firmware supports **assigning function aliases to any general-purpose DI** at `:9198 → Configuration → IO` — "Switch to Auto Mode", "Switch to Manual Mode", "System Reset", "Program Start/Pause/Stop", "Protective Stop" are all documented aliases. Bound DI's **rising edge** fires the alias action; effect is immediate (no Save, no restart). Combined with the already-shipped `IOManager/SetIOForcedFlag` wire path, this gives us zero-wire mode switching without touching the DI16 hardware modeSwitch gate at all. **DI16 hunt retired** — reserved-input status remains, but we no longer need to solve for it. **Shipped:** (1) `ESTUN_MODE_VIA_DI` env on roboai-estun (default 0); when 1, driver's `_on_mode_command` replaces `Robot/toAuto`/`toManual` with a `SetIOForcedFlag` pulse on the bound DI (defaults DI6/DI7, overridable via `ESTUN_MODE_DI_AUTO`/`ESTUN_MODE_DI_MANUAL`, pulse width `ESTUN_MODE_DI_PULSE_MS` default 120 ms); envelope carries `via='bound_di_<port>'`. `to_remote` stays on WS verb (no bound-DI alias for Remote in manual). (2) Dashboard `/api/estun/mode` Rung 0 (DI16 pre-check) auto-skips when `ESTUN_MODE_VIA_DI=1` — otherwise a system on the bound-DI path would trip the DI16 hardware gate even though the software path bypasses it. (3) `/tmp/probe_mode_via_di.py` stages TEST A (pulse DI6→watch mode 1→0; pulse DI7→watch mode 0→1) for operator execution once bindings land. **PLC-integration story:** this DI function-binding surface IS the product's PLC integration path — any customer PLC drives Run/Stop/Reset/mode/Protective-Stop via general DIs bound to their function aliases in the factory UI, no software required. Added to `HARDWARE.md > Synapse enclosure note` as a first-class controls-engineer-deck slide. **Also encoded:** codroidsafety admin password (factory UI Configuration/IO-binding gate), Simulation vs Real Machine switch (existence noted, details TBD), error-code appendix reference — all in `HARDWARE.md > Software-manual constants`. **Also queued:** Patrick to request Estun "Remote Control Manual" + "Register Protocol Table" — Modbus basic-control registers may replace bound-DI as the shipped-unit integration channel; added to STATE.md `Outstanding requests (external)`. — 2026-08-31 software manual absorb

314. CC10-A manual absorbed — DI-block wiring plan RETRACTED, safety §5.2.2 confirmed (2026-08-31). Four facts landed from the operator's manual read. **(1) DI16/17/18 provenance:** general-purpose DIs are physically DI0–DI15; the "DI16/17/18" labels seen in `IOManager/GetIOInfo` are RESERVED signals routed through dedicated interfaces (DI18 = flange drag via aviation plug — long-known per add-12; DI16 modeSwitch and DI17 enableButton are routed via the Hand Controller interface, not the general I/O terminal block). My prior HARDWARE.md guidance suggesting a "cabinet-side jumper" for DI16 is retracted — that terminal doesn't exist on the general block. Correct forward path: manual §4.2.3 documents **two definable function keys on the Hand Controller** whose functions "can be set in the setting interface"; check whether one of those keys can be assigned as modeSwitch from the factory UI settings screen. If YES → zero-wire fix. If NO → pull HC interface pinout from the manual + probe the connector before any hardware plan. **(2) Cabinet light-strip mode indicator:** Blue = Manual mode AND arm enabled; Green = Auto OR Remote mode. Driven directly from the wire, so any dashboard-mirror disagreement means trust the strip. **(3) Cabinet power controls — TWO distinct switches:** the ON/OFF ROCKER cycles servo power only (fires alarm 9012, WS session survives, does NOT reset recoveryState — this is what the operator has been actuating on prior "power-cycle" attempts); the POWER KEY is the true mains-side CPU cycle (WS session drops, new `Connected ws` line in `journalctl -u roboai-estun`, new `/opt/cobot/logs/estun_ws_*.jsonl` rotation). This resolves the 2026-08-31 "power-cycle didn't clear the latch" mystery — the rocker was cycled, not the key. Verification recipe: `curl -sI http://192.168.2.136:9198` during the OFF window returns connection-refused when the CPU is actually down (the factory UI is served by the CPU). **(4) Safety §5.2.2 confirmed:** 4 dual-channel inputs (ch1-2 = protective/interlock, ch3-4 = emergency stop), internal safety relays (no external relay required for standard integration), Cat-1 e-stop (controlled stop → power removal), 24V drop output that removes servo enable independently of any wire verb, **ships with factory jumper shorts across all safety input pairs** so the cabinet runs out of the box — these MUST be removed before shipping any customer integration (intact factory short = safety chain permanently satisfied = no safety chain). All slide-4 TO-CONFIRM items on the safety-relay deck now flip to CONFIRMED-BY-MANUAL with §5.2.2 citation; bench session re-scoped from "confirm-what-it-is" to (A) behavioral verification of dual-channel discrepancy detection + timing, and (B) factory-short removal procedure with before/after photos as sign-off (see `docs/safety_relay_deck_slide4.md`). Also landed: `docs/product_maintenance.md` starter with monthly safety functional tests (e-stop press-and-verify, drag button MANUAL-only test, safety I/O both-channels test, mode light-strip sanity) — carries forward when the customer-facing maintenance manual is authored. FACTS.md + HARDWARE.md updated in-place; memory `cobot-di16-mode-selector` amended to retract the jumper hypothesis and point to the HC-function-key path as the actual next step. — 2026-08-31 manual absorb

313. DI16 modeSwitch + recoveryState reframe (2026-08-31, add-53 §655-662). Wire caught two doctrine errors in one operator session. **(1) recoveryState reframe:** wire evidence during the MODE pill acceptance run showed `recoveryState` transitions `0→1` at the moment of `Robot/switchOff` and stays `1` for the rest of the CPU session — it is a session-persistent servos-were-off flag, NOT a fault latch. The addendum-40 §566 "physical cabinet power-cycle is the ONLY remedy" doctrine was based on a tuple `{state:2, recoveryState:1, errors:[]}` that every operator session trivially reaches after any disable. Mode-ladder Rung 1 (`rs != 0 → power-cycle_required`) was over-firing on normal conditions; it is DELETED. The real power-cycle-only condition is Rung 2's failure mode (`errors_latched_uncleared` — errors\[\] non-empty AND `System/ClearError` didn't drain within 2 s). `recoveryState` is retained in every `four_tuple` for observability but never gates a refusal on its own. **(2) DI16 modeSwitch discovery:** `IOManager/GetIOInfo` reveals `DI16` factory-named `modeSwitch`. The firmware silently ACKs `Robot/toAuto` (empty `db`, success shape) but does NOT transition mode when `DI16 == 0`. Shipped as Rung 0 pre-check in `dashboard_server.py:7534` — reads `STATE['io_live']` (driver polls at ~500 ms), only gates `target == 'auto'`, emits `mode_selector_manual` outcome with the honest physical-action copy ("Mode selector at the cabinet is in MANUAL — turn the physical selector to AUTO"). Stale-mirror never blocks (io_live absent → let orchestration try). **(3) §644 palletize verdict:** was NEVER a firmware latch. The observed refusals were DI16=0 (silently no-op'ing toAuto after palletize's push chain called it) stacked on rs=1-as-session-normal. The offline §644 finding stands (transit_over_slot IK-refuse + partial expansion in `program_ops.py:3882+`), so the quarantine survives per add-52 belt-and-braces, but the `_QUARANTINE_REASON` string updates to name the codegen defect, not the mythical "recovery-state class." Every physical cabinet cycle burned chasing "the palletize latch" was answering a question the wire was not asking. **(4) Doctrine tests inverted:** `test_mode_endpoint_has_di16_rung_zero` (positive Rung 0 pin) and `test_mode_endpoint_does_not_gate_on_recovery_state_alone` (negative — retired reason_code/kind MUST NOT be emitted). `namedModeError` retired branch's copy explicitly names the legacy status ("Legacy refusal — this backend version is out-of-date") and cites add-53. HARDWARE.md > "I/O vocabulary" gains DI16 semantics + a Synapse enclosure note flagging DI16 as a controls-engineer-deck panel-feature + risk-assessment line item (five-zone enclosure must expose/relocate this as an operator-accessible physical selector). **Blast-radius sweep:** no self-poisoning found; the mode ladder's own Rung 3 orchestration SETS rs=1 via `Robot/switchOff`, but nothing downstream read rs after that point — the bug lived entirely in the never-should-have-existed Rung 1. — 2026-08-31 wire evidence

312. Monitor page cleanup + speed model (2026-08-31 UI directive). Four scoped changes on the legacy Lua run console. **(1) Header cleanup:** latency "N ms" chip retired from `TopBar.jsx` (wsLatency still tracked in the store for diagnostics but no longer furniture). **(2) Run console cleanup:** the Demonstration/weight/demo-id badges and the program-description IIFE removed from `MonitorDashboard.jsx` (the `ProgramProvenance` component + `readPayload`/`payloadChipLabel` imports deleted); this metadata lives in Program Library detail view, not the run console. `has_taught_poses === false` still surfaces via the Run modal's `pending_poses` named refusal — nothing operator-blocking was lost. **(3) Provenance relocation:** the footer "served <hash>" pill (`FooterBuild`) retired from `StatusBar.jsx`; a new `ProvenanceSection` in `ConfigureLayout.jsx` exposes verdict + deploy_sha + backend_sha + frontend_sha + served bundle hash. `DeployStatusBanner` now renders null on `verdict === 'current'` (green) — the footer only surfaces when verdict ≠ green. `/health` continues to expose `backend_sha`/`frontend_sha`. StaleGuard overlay is unchanged (still blocks on backend/frontend SHA mismatch). Enforcement chain is unchanged; only the render location moves. **(4) Enable/Disable single stateful button:** `ArmEnableControl.jsx` rewritten from chip+state-label+button combo into ONE button driven by wire state (`state_code === 2 numeric` per FACTS.md, boolean `robot.enabled` fallback for older builds). Label: "Enable" (green) when disabled, "Disable" (red) when enabled, "Enabling…" (amber, spinner) during the transition. Confirm dialog on Enable only — motion becomes possible there; Disable is non-destructive from a "did I mean it" standpoint. **(5) Speed model:** artificial UI cap removed from `ProgramSpeedEntry` (full 1..100 accepted; controller-side `operator_speed_limit` refuses via `namedSpeedRefusal` if exceeded, per add-51 §638-645). Per-program persistence: each program record stores its own `config.speed_pct`; `setRunSpeedPct` debounces (400 ms) an auto-save via PUT `/api/programs/{id}` with config-merge (preserves payload, motion_profile, etc.). Switching programs loads that program's saved speed; new / never-run programs default to 25% via `setCurrentProgram`'s `patch.id !== undefined` branch. The F2.7 first-run rule is now a DEFAULT, not a hard cap — the operator can set whatever the controller will honor. Doctrine tests: 7 frontend source-grep pins in `useStore.programSpeed.test.js` (default 25, debounce+program-scope, PUT+config-merge, local mirror, load-on-id-change, UI-cap absence); 4 server-side on-disk tests in `test_per_program_speed_persistence.py` (new-program no stored speed, merge preserves other config, save scoped to one program, reload returns 60). All 11 doctrine tests pass; earlier `test_load_pushes_to_controller` (11/11) and `test_operator_refusal_copy` (17/17) suites still green. — 2026-08-31 UI directive

311. Mode-refusal copy unified through `namedModeError` (2026-08-31, session sha 7a8e312 tab). Symptom: every program run refused with the OLD generic "mode read-back timeout" copy on the current tab. Diagnosis: routing was correct — `RunProgramModal.confirmRun` and `ModeControl.runSwitch` both already POST to `/api/estun/mode` (the ladder endpoint from add-51 §638-645). The bug was **copy-flattening** at BOTH call sites — `body?.outcome?.reason || body?.outcome?.detail || "HTTP N"` collapsed the ladder's per-rung outcome (`recovery_state_power_cycle_required` → physical cabinet cycle, `errors_latched_uncleared` → pendant :9198, `driver_ack_timeout` → check roboai-estun, `mode_switch_failed` with `reason_code=mode_readback_timeout` → check pendant key-switch) into a single string. `RunProgramModal` compounded it with a hardcoded title *"Can't start — controller is not in Auto and the switch was refused"* wrong for Rung 1 (recoveryState=1 has nothing to do with Auto). No frontend code recognized any ladder `outcome.kind` or `reason_code`; no `namedModeError` helper existed. Fix: `frontend/src/lib/modeOutcome.js` with `namedModeError(body, httpStatus)` — named branches for every kind + every `mode_switch_failed` `reason_code` — plus the `four_tuple` preserved verbatim in `technicalDetail`. Both call sites rerouted. Fork registry entry `mode_refusal_copy` (23b) locks it forward; `test_every_estun_mode_call_site_imports_named_mode_error` is an exhaustive-fetch-site sweep so a third call site can't reintroduce the flattening. Doctrine tests 17/17. Also confirmed by add-49 §621-629 predicting this: the mode pill acceptance has been on the list for three sessions; program-run attempts became the de-facto first live test of mode machinery — this was the wrong first test. Directive: run the pill acceptance deliberately BEFORE any program run, with the tuple probed first (Rung 1 hard-stops instantly if `recoveryState!=0`, no code fix helps until the cabinet is cycled). — 2026-08-31 UX diagnosis

320. Device pairing is the security boundary, not a UX flourish. Before add-57, dashboard shipped with no auth surface at all: FastAPI monolith at :8080, CORS `allow_origins=["*"]`, every /api/* + /cmd/* + /stream/* + /ws/* endpoint accepted anyone on the LAN. Threat model before: anyone with LAN access moves the arm, reads state, deletes programs — the plaintext-credential class was the whole story. Add-57 backend slice (default-OFF): (a) `/opt/cobot/identity.json` — serial `NR-XXXXXX` minted once, model + friendly_name, immutable; (b) `pairing.py` PairingStore — in-memory pending sessions (90 s TTL, single-use codes), on-disk `paired_devices.json` (SHA-256 hashed 256-bit tokens, raw returned once), 3-fail-per-5-min → 5-min IP lockout, revoke pubsub; (c) HTTP + WS auth middleware gated by `COBOT_PAIRING_ENFORCED` env — inert this session so nothing breaks, but wired end-to-end on every /api/*, /cmd/*, /stream/* HTTP route + all 10 /ws/* handlers, unauth allow-list for the pair flow itself + /health + /api/deploy_status + /api/provenance + localhost. Revoke closes live WS via `_ws_close_by_token` scheduled on the FastAPI event loop. Fork-registry entry #27 pins: no second token store, no parallel auth middleware, no ad-hoc /pair/* endpoint. Frontend wizard + Avahi `_neurobots._tcp` advertisement scoped to follow-on sessions layered on THIS module — enforcement flag flip is a one-line operator step. — 2026-09-18 add-57 §686

319. Untested-plugin-on-live-arm activation without operator MARK is a chain-of-command breach that a "commit + deploy" directive does NOT authorize. 2026-09-01 shake event traced to this class: the add-55 §681 traj-start reset fixed the step-2 halt in unit tests + mock replay, but was activated onto the live enabled arm's `ros2_control_node` write() cycle via CRI relaunch WITHOUT an operator MARK for plugin activation. New plugin code executed at 250 Hz on the live arm without ever having touched hardware. The bug in that traj-start reset (fires every idle cycle when JTC-hold ≈ pos_state within eps, overwrites `pos_cmd_prev_sent_`) destroyed clamp_accel_step's history baseline and produced 129 accel_clamp engagements/s at the shake window — mechanism CONFIRMED via mock reproduction; whether it was the sole physical shake cause is unattributable without a joint-velocity trace that wasn't captured (real forensics gap). Fix batch (add-56 §683): (a) traj-start reset rewrite fires ONCE on `command_synced_ 0→1` edge or `/cri/trajectory_start` signal, never touches `pos_cmd_prev_sent_`; (b) `tools/mock_replay.py` writes `docs/mock_replay/<sha256>.json` artifact keyed by installed plugin .so sha256; (c) `tools/preflight_plugin_check.py` gates activation on that artifact + arm-preflight charter grew from four checks to five; (d) three-token MARK chain (`plugin_activation`, `dry_pass_with_arm_enabled`, `real_fire`) in `.marks/`, each single-use + sha-bound; `launch_cri_with_mark.sh` consumes plugin_activation, executor consumes the other two; (e) `cri_tcp_setup` `response_looks_ok()` now rejects `"err":` fields — the launch that silently declared "全部 5 步 TCP 初始化成功" on a rejected StartControl is now an explicit `phase=start_control_rejected` failure. — 2026-09-01 add-56 §682-685

318. F2.7 FIRST-FIRE ACHIEVED (2026-09-01, add-55 §679-680). ROS2 executor pipeline dispatched a Pilz PTP joint plan to the S10-140 via JTC and the arm physically moved to Test100 taught_joints exactly. Yesterday's "J1/J6 convention drift" was a bug in an ad-hoc probe: `.strip("- \n")` on a `"- -0.1926"` line stripped BOTH the list-item `-` prefix AND the numeric `-` sign, so every negative reading came back positive; correct parser is `.replace("- ", "", 1).strip()`. Wire is coherent: WS `publish/RobotPosture`, JSB `/joint_states`, and estun_driver `/joint_states` all agree to 0.001° at the same physical pose; FK-TCP from JSB matches WS `end` within 15.5 mm (the Phase E5 Estun-vs-URDF frame offset baseline). Frame-coherence pre-flight gate + per-step post-motion cross-check + JTC Humble `status=UNKNOWN + rc=0` acceptance shipped alongside. What blocks full 8-step Test100 completion: cri_hardware's `hold_if_cmd_far_from_fb_rad = 0.20 rad (11.5°)` guard trips mid-trajectory when servo-following error saturates during longer joint sweeps (step 2 approach-pick, ~30° J1 travel at 25% cruise 0.6545 rad/s); 15% retry failed too because `pos_cmd_sent_` state carries the frozen offset across trajectories and can only catch up at `max_step_rad` per cycle. Fix options deferred to next session per §680. — 2026-09-01 add-55 §679-680

317. Ad-hoc probe scripts are load-bearing: verify their parser first. Every conclusion about "the arm is lying" for hours yesterday came from `.strip("- \n")` silently zeroing the minus sign on `ros2 topic echo` output. Tools that transform wire data before comparison are as much a suspect as the wire itself; when two independent channels disagree at the byte level, dump a raw sample from BOTH before writing a diff script. Guard against this class systematically: the executor's new `_frame_coherence_ok()` gate uses websockets library's JSON parse (correct) + rclpy's typed `JointState` message (correct) — no string parsing of joint numerics. — 2026-09-01 add-55 §679

310. Resident-mismatch banner retired (2026-08-31 UX directive). Selection IS the sole push trigger — explicit program selection auto-pushes to the controller silently (small transient "Loaded X" toast on success; named toast on failure via `namedLoadError`). NO auto-push on reconnect, boot, tab-restore, or `ui_context` drift — drift-push was the class that re-poisoned the resident last week. Quarantined programs (423) refuse at pick time with the named `quarantined` outcome so a selection never half-succeeds into a mismatch state (`loadOutcome.js` + `dashboard_server.py:6553/6598`). Server-side mismatch DETECTION stays as the add-29 log/event class only — no banner ever renders; if it fires without a drift cause, we want the event, not UI. Scope: this is minimal glue on the legacy path, NOT new architecture; the whole resident-program concept retires with `RUN_BACKEND=ros2_executor` (add-52 §646-654) since CRI streaming has no resident. Retired from `MonitorDashboard.jsx`: `residentDivergence` derivation, `<div data-testid="resident-divergence">` banner, `onBannerPushToController` handler, `banner-push-to-controller` button. Pinned tests inverted from "banner MUST render" to "banner MUST NOT render"; `test_load_pushes_to_controller.py` reflects the new invariant. — 2026-08-31 UX directive

305. Stale class CLOSED — provenance chain enforced end-to-end, SHA-to-SHA at every layer (L257 like-for-like). Backend bakes `_BACKEND_GIT_SHA` + `_BACKEND_START_ISO` at import (env-first, git-fallback); frontend bakes `__GIT_SHA__` via vite `define` + writes `dist/.build-sha` sidecar (writeBundle plugin). `/api/provenance` is canonical (mirrored on `/health`). `/api/deploy_status` composes a three-layer verdict — never green while deploy_log, backend, or frontend SHAs disagree; `failing_layers` names the drifter. `/ws/state` pushes `{type:'hello', backend_sha, frontend_sha}` immediately after accept; frontend `StaleGuard` is a BLOCKING full-screen overlay (aria-modal, no dismiss control, only Reload-now button — dismissible-toast pattern retired because the operator had learned to click through). `deploy.sh` + `autodeploy_wrapper.sh` refuse a dirty tree unless `ALLOW_DIRTY=1` (ALLOW_MOCK pattern); refusal writes `phase=fail, step=dirty_tree_refused` to deploy_log so the banner names it. Post-restart deploy asserts `/api/provenance.backend_sha == git rev-parse HEAD` (kills the surviving-old-worker ghost) and served `frontend_sha == HEAD` (kills the stale-bundle class the FRONTEND_OUT drift used to hide). 18-test doctrine (source-grep) + fork_registry `provenance` capability lock everything against silent regression. Trigger for the whole overhaul: Test100 save's "transient network hiccup" toast on what was a driver `allow_move gate closed` reject — 4.05 s race in the save-classifier polling loop compounded by FRONTEND_OUT drift (`mock_server/static/` → `frontend/dist/` migration `b1729b4` never propagated to deploy scripts) and a `_state_lock = threading.Lock` self-deadlock from the twin phantom-feedback fix (`_on_estun_status:2131` → `_wire_arm_is_enabled:229` under `JOG_BACKEND=ws` + all-zero joints). Acceptance A/B/C green on session shas `0563a83..c5d697f`. — add-48 §613-620
